...
try {
    ...
    try {
        ...
        # Simple version check (parsing "Python 3.x.y")
        if ($pyVer -match "Python (\d+)\.(\d+)") {
            $major = [int]$matches[1]
            $minor = [int]$matches[2]
            if ($major -lt 3 -or ($major -eq 3 -and $minor -lt 8)) {
                throw "Python 3.8+ is required. Found version is too old."
            }
        }
    } catch {
        throw "Could not execute '$PythonVersion'. Please ensure Python 3.8+ is installed and in your PATH."
    }
    ...
    switch ($CudaVersion) {
        ...
        "cpu"  { $TorchUrl = "cpu" } # Default index usually serves cpu, but specifying ensures no mixup
    }
    
    $InstallCmd = "$TargetPip install torch torchvision torchaudio"
    if ($CudaVersion -ne "cpu") {
        Write-Host "   Targeting CUDA $CudaVersion..." -ForegroundColor Gray
        & $TargetPip install torch torchvision torchaudio --index-url $TorchUrl --quiet
    } else {
        Write-Host "   Targeting CPU only..." -ForegroundColor Gray
        & $TargetPip install torch torchvision torchaudio --quiet
    }
