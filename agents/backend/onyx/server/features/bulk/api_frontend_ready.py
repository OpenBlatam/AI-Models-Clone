...
# Intentar importar componentes del sistema BUL
try:
...
# Intentar importar Prometheus metrics
try:
...
# CORS Configuration - Permitir todas las conexiones desde frontend
app.add_middleware(
    CORSMiddleware,
...
# Inicializar procesadores BUL si están disponibles
    ...
    try:
        ...
        if BUL_AVAILABLE and truthgpt_processor:
            try:
                ...
                # Procesar con actualizaciones de progreso
                task["progress"] = 20
                await broadcast_task_update(task_id, {"status": "processing", "progress": 20})
                
                # Generar contenido usando el procesador real
        ...
        # Modo simulación si BUL no está disponible o falló
...
# Métricas Prometheus (si están disponibles)
