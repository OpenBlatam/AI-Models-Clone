...
class VideoGenerationPipelineService:
    ...
    async def _generate_speech(
        ...
        try:
            ...
            voice_config = VoiceGenerationConfig(
                quality=quality_map.get(request.video_quality, VoiceQuality.HIGH),
            )
    ...
    async def _render_final_video(
        ...
        try:
            ...
            # Map quality and format strings to enums
            quality_map = {
            ...
            video_config = VideoConfig(
                resolution=request.resolution,
                fps=30,
                quality=quality_map.get(request.video_quality, VideoQuality.HIGH),
                format=format_map.get(request.output_format, VideoFormat.MP4),
...
class HeyGenAISystem:
    ...
    async def generate_avatar_only(
        ...
        avatar_config = AvatarGenerationConfig(
            style=style_map.get(style, AvatarStyle.REALISTIC),
            quality=quality_map.get(quality, AvatarQuality.HIGH),
    ...
    async def generate_speech_only(
        ...
        voice_config = VoiceGenerationConfig(
            quality=quality_map.get(quality, VoiceQuality.HIGH),
        )
