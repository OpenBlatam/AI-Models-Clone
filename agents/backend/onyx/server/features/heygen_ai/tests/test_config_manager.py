...
"""
    ...
    def test_load_environment_variables(self, temp_config_dir):
        ...
        assert config_manager.database.host == "env-host"
        assert config_manager.database.port == 5434
        assert config_manager.api.port == 9001
    ...
    def test_config_validation_failure(self, temp_config_dir):
        ...
        assert "Database port must be between 1 and 65535" in error_message
        assert "API port must be between 1 and 65535" in error_message
        assert "Secret key cannot be empty" in error_message
    ...
    def test_get_config(self, temp_config_dir):
        ...
        assert config_dict['api']['port'] == 8000
        assert config_dict['system']['environment'] == "development"
    ...
    def test_save_config(self, temp_config_dir):
        ...
        # Modify some values
        config_manager.database.host = "saved-host"
        config_manager.api.port = 9000
        ...
        # Load the saved configuration to verify
        new_config_manager = ConfigManager(temp_config_dir, skip_validation=True)
        new_config_manager.config_file = output_file
        new_config_manager._load_config()
        
        assert new_config_manager.database.host == "saved-host"
        assert new_config_manager.api.port == 9000
    ...
    def test_reload_config(self, temp_config_dir):
        ...
        config_manager.database.host = "modified-host"

        # Reload configuration
        config_manager.reload_config(skip_validation=True)

        # Since there's no config file, the value should remain modified
        # This tests that reload_config doesn't crash and handles missing files gracefully
        assert config_manager.database.host == "modified-host"
