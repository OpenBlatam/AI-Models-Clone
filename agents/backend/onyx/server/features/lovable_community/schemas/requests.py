...
class SearchRequest(BaseModel):
    ...
    tags: Optional[List[str]] = Field(
        None,
        max_length=10,
        description="Filtrar por tags (máximo 10)"
    )
    ...
    def validate_tags(cls, v: Optional[List[str]]) -> Optional[List[str]]:
        ...
        if len(v) > 10:
            raise ValueError("Maximum 10 tags allowed for filtering")
...
class UpdateChatRequest(BaseModel):
    ...
    tags: Optional[List[str]] = Field(
        None,
        max_length=10,
        description="Nueva lista de tags (máximo 10)"
    )
