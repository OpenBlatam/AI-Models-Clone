...
class DefectClassifier:
    ...
    def classify_defects(
        ...
        # Detección adicional de defectos específicos
        scratch_defects = self._detect_scratches(img)
        defects.extend(scratch_defects)
        
        crack_defects = self._detect_cracks(img)
        defects.extend(crack_defects)
        
        dent_defects = self._detect_dents(img)
        defects.extend(dent_defects)
        
        discoloration_defects = self._detect_discolorations(img)
        defects.extend(discoloration_defects)
        
        missing_part_defects = self._detect_missing_parts(img)
        defects.extend(missing_part_defects)
        
        contamination_defects = self._detect_contamination(img)
    ...
    def _classify_anomaly_as_defect(self, anomaly, image: np.ndarray) -> Optional[Defect]:
        ...
        severity = self._determine_severity(anomaly.confidence, area, defect_type)
        ...
        defect = Defect(
            ...
            severity=severity,
            area=area,
            description=f"{defect_type.value} detected: {anomaly.description}",
        ...
        try:
            ...
            if lines is not None:
                for line in lines:
                    ...
                    if length > 30:  # Línea suficientemente larga
                        ...
                        if confidence >= 0.5:
                            severity = self._determine_severity(confidence, area, DefectType.SCRATCH)
                            ...
                            defect = Defect(
                                ...
                                severity=severity,
                                area=area,
                                description=f"Scratch detected (length: {length:.1f}px)",
            ...
            for contour in contours:
                ...
                if aspect_ratio > 3.0:
                    ...
                    if circularity < 0.3:
                        ...
                        if confidence >= 0.5:
                            severity = self._determine_severity(confidence, area, DefectType.CRACK)
                            ...
                            defect = Defect(
                                ...
                                severity=severity,
                                area=area,
                                description=f"Crack detected (aspect ratio: {aspect_ratio:.2f})",
                                features={"aspect_ratio": float(aspect_ratio), "circularity": float(circularity)}
                            )
                            defects.append(defect)
        ...
        try:
            ...
            for contour in contours:
                ...
                if circularity > 0.5:  # Relativamente circular
                    ...
                    if confidence >= 0.5:
                        severity = self._determine_severity(confidence, area, DefectType.DENT)
                        ...
                        defect = Defect(
                            ...
                            severity=severity,
                            area=area,
                            description=f"Dent detected (circularity: {circularity:.2f})",
    ...
    def _detect_discolorations(self, image: np.ndarray) -> List[Defect]:
        ...
        try:
            ...
            for contour in contours:
                ...
                if confidence >= 0.5:
                    severity = self._determine_severity(confidence, area, DefectType.DISCOLORATION)
                    ...
                    defect = Defect(
                        ...
                        severity=severity,
                        area=area,
                        description="Discoloration detected",
        ...
        try:
            ...
            for contour in contours:
                ...
                if circularity > 0.6:
                    ...
                    if confidence >= 0.6:
                        severity = self._determine_severity(confidence, area, DefectType.MISSING_PART)
        ...
        try:
            ...
            if len(peaks) > 0:
                ...
                for contour in contours:
                    ...
                    if confidence >= 0.5:
                        severity = self._determine_severity(confidence, area, DefectType.CONTAMINATION)
    ...
    def _remove_duplicates(self, defects: List[Defect]) -> List[Defect]:
        ...
        for defect in sorted_defects:
            ...
            for existing in filtered:
                ...
                # Si hay superposición significativa (>50%), es duplicado
