...
class DefectAnalyzer:
    ...
    def _analyze_sizes(self, defects: List[Defect]) -> Dict:
        ...
        return {
            ...
            "small_defects": sum(1 for a in areas if a < 500),
            "medium_defects": sum(1 for a in areas if 500 <= a < 2000),
            "large_defects": sum(1 for a in areas if a >= 2000)
        }
    ...
    def _determine_status(self, defects: List[Defect], coverage_percentage: float) -> str:
        ...
        if critical_defects:
            return "critical"
        elif severe_defects and len(severe_defects) > 3:
            return "severe"
    ...
    def _generate_recommendations(
        ...
        # Verificar tipos de defectos más comunes
        if type_analysis:
            most_common_type = max(type_analysis.items(), key=lambda x: x[1]["count"])
            if most_common_type[1]["count"] > 3:
