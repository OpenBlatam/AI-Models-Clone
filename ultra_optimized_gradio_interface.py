...
class UltraOptimizedGradioInterface:
    ...
    def _create_image_generation_tab(self):
        ...
        with gr.Column():
            ...
            if self.config.show_examples:
                gr.Examples(
                    examples=[
                        ["A beautiful sunset over mountains", "blurry, low quality"],
                        ["A futuristic city with flying cars", "dark, scary"],
                        ["A serene forest with sunlight", "urban, noisy"],
                        ["An astronaut floating in space", "earth, ground"]
                    ],
        ...
        with gr.Column():
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    with gr.Row():
                        layer = gr.Slider(
                            minimum=0, maximum=12, value=0, step=1,
    ...
    def _create_batch_generation_tab(self):
        ...
        with gr.Column():
            gr.Markdown("## Batch Image Generation with Ultra-Optimized Diffusion Models")
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    negative_prompts_input = gr.Textbox(
                        label="Negative Prompts (one per line, optional)",
                        placeholder="Enter negative prompts, one per line...",
                        lines=3
                    )
                    
                    generate_batch_btn = gr.Button("Generate Batch Images", variant="primary")
            ...
            if self.config.show_examples:
                gr.Examples(
                    ...
                    inputs=[prompts_input, negative_prompts_input]
                )
    ...
    def _batch_generate_wrapper(self, prompts_text: str, negative_prompts_text: str = "") -> List[Image.Image]:
        """Wrapper for batch generation that parses text inputs."""
        try:
            # Parse prompts
